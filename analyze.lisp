...
(def-instruction-analyzers (assign cast-as) (:local :temp)
  (let ((value (instruction-arg1 instruction))
        ...
        (made-change nil))
    (when (and (papyrus-constant value)
               set)
      (loop for instruction2 in set
            unless (sibling-intersection instruction2 siblings)
            do (rewrite-arguments instruction2 identifier value)
               (setf made-change t))
      made-change)))
